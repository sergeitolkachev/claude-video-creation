...
- Verticals end on a hard cut mid-sentence. **Captions are mandatory** — a
  vertical is watched muted first and read second, so a cut without them is
  not finished. They are burned in from the real word timings
  (`build_captions.py`, ElevenLabs with-timestamps, called with the same seed
  *and* the same previous_text as the take in the mix), never typed by hand
  and never auto-generated by the platform.
- **A vertical is assembled, not carved.** Records 01 to 03 took one window
  out of the master — a `from`, a `to`, the record's own order and a centre
  crop that was not a choice but a constant in the source. That throws away
  the two things that decide whether a cut is watched. The record's order is
  paced for somebody who has already chosen to watch; a cut has four seconds
  to earn that. And the centre of a 16:9 frame is not where the subject is —
  a hatch to the left, a gauge in the right third, and the centre crop
  delivers the wall between them. From record 04 a cut is built the way the
  teaser is built: a shot list out of `takes/` in whatever order works, each
  shot framed where its subject is, and a line list out of the paragraphs the
  record already contains. `verticals.yaml` holds all three, and
  `scripts/vertical.py` is the one assembler both it and the teaser go
  through.
- **Frame each shot where the subject is.** `crop_x` is the 608 px window's
  centre as a fraction of the frame width — 0.5 is the old centre crop and
  the default. `crop_to` makes the window travel across the shot, which is a
  pan inside a photograph and the only camera move a cut is allowed. Free,
  exact, and worth more than any transition.
- **Hard cuts only. No dissolves, no dips, no wipes.** A dissolve says a
  person edited this, which is the one thing a recovered recording cannot
  afford, and a dip to black reads as a dropped frame in the player for the
  same reason the tape dropouts did. What buys the attention instead is short
  shots, a subject that physically moves, and a crop that frames it. If a cut
  needs a transition to hold, the shot order is wrong.
- **Nothing is generated for a vertical.** Every shot is a take already in
  `takes/`; every line is a paragraph already approved and reproducible from
  its seed. A shot may be used twice; a sentence the record does not contain
  may not appear at all.
- **No music under a vertical cut.** The teaser is the only piece on this
  channel allowed a cue, and that exemption does not spread. The room tone
  runs under the whole cut, including under the closing card, and its level
  is inherited from the record's own `audio.yaml` so the cut sounds like the
  record — measured on record 03, the assembled form lands its floor within
  0.3 dB of the master's.
- **`runtime_seconds` means the whole piece, closing card included**, in
  `verticals.yaml` and in `teaser.yaml` alike. The first version of the
  assembled builder read it as picture-only in one file and total in the
  other: the same key, on the same kind of object, measuring two different
  things. `validate.py` checks the shot list sums to it.
- **Look at the finished thing.** Every gate in this repository reads a file
  and scores it, and none of them looks at the picture — which is how record
  02 shipped three cards cut off mid-word and record 03 shipped a silent
  typewriter, each one arithmetically defensible and each one obvious at a
  glance. `scripts/contact_sheet.py` builds that glance: two frames per shot
  off the 16:9 master, labelled with time and shot id, and an even sample of
  every 9:16 piece with `bottom_clear` and `safe_width` drawn on it. Run it
  before publishing and look at it. A check that passes is not a picture that
  works.
...
- **Assembled, not excerpted.** The teaser takes the shots where something
  physically moves — a hatch closing, dust in a beam, a trace climbing a
  screen — and reorders them freely. `teaser.yaml` holds the shot list, the
  line list and the levels. Since record 04 the vertical cuts are built the
  same way, through the same `scripts/vertical.py`, so this is no longer what
  separates the two. What separates them is the bed and the card: the teaser
  is the only piece allowed music under it, and it closes on `SOON`.
...
- Verticals carry no data cards. A card is composed for the lower left of a
  ...
  survive and the numbers do not. Records 01 to 03 got this by cutting from
  `EPISODE-clean.mp4`; an assembled cut gets it for free, because it comes
  from `takes/`, which never had a card or a click on it in the first place.
  Either way the click track goes out with the cards, because a typing sound
  over nothing typing is a fault — and every figure that matters has to be
  spoken aloud, or it does not survive the crop.
